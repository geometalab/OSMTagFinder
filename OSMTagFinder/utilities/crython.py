...
minu = lambda v: CronField(v, 0,    59,   set(['*', '/', ',', '-']))
...
class CronExpression(object):
    ...
    FIELDS = dict(zip(FIELD_NAMES, (sec, minu, hr, dom, mon, dow, yr)))                      #field name->init func
    ...
    def __init__(self, **kwargs):
        ...
        #correction @Simon Gwerder
        expr = kwargs.get('expr')
        if expr in self.KEYWORDS:
            expression = self.KEYWORDS[expr]
        else: # actually would have to valid check pattern here.
            expression = expr
